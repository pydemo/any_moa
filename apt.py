...
async def predict_large_language_model_sample(
    project_id: str,
    model_name: str,
    temperature: float,
    ...
    response = await asyncio.to_thread(chat.send_message, "What version of PaLM are you?", **parameters)
...
async def main():
    ...
    try:
        response = await predict_large_language_model_sample(
